...
// In browser, we would use the worker, but in Node.js it causes issues
if (typeof window !== 'undefined') {
  // Running in browser - use worker
  // @ts-ignore
...
export const readPdf = async (fileUrl: string): Promise<TextItems> => {
  // For Node.js, disable worker to avoid "fake worker" errors
  const loadingTask = typeof window === 'undefined'
    ? pdfjs.getDocument({ url: fileUrl, useWorkerFetch: false, isEvalSupported: false, useSystemFonts: true })
