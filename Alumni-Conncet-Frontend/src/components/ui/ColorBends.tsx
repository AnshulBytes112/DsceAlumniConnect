...
type ColorBendsProps = {
  ...
  noise?: number;
  dpr?: number;
  skipFrames?: number; // New prop to control frame skipping
};
  ...
  q += toward * uMouseInfluence * 0.2;
    ...
    if (uColorCount > 0) {
      ...
      for (int i = 0; i < MAX_COLORS; ++i) {
            ...
            // Optimized: Replaced expensive double exp with smoothstep
            // Original: float w = 1.0 - exp(-6.0 / exp(6.0 * m));
            float w = 1.0 - smoothstep(0.0, 0.3, m);
      ...
      a = uTransparent > 0 ? cover : 1.0;
        ...
        for (int k = 0; k < 3; ++k) {
            ...
            // Optimized
            col[k] = 1.0 - smoothstep(0.0, 0.3, m);
        }
...
export default function ColorBends({
  ...
  noise = 0.1,
  dpr = 1,
  skipFrames = 0 // Default 0 means render every frame. 1 means skip 1 frame (render every 2nd), etc.
}: ColorBendsProps) {
  ...
  const frameCountRef = useRef<number>(0); // To track frames for skipping
  ...
  }, [dpr, skipFrames]); // Re-create if dpr or skipFrames changes
  ...
  useEffect(() => {
    ...
    // Optimization: Only attach listener if mouseInfluence is significant
    if (mouseInfluence <= 0.001) return;
  ...
  }, [mouseInfluence]); // Re-run if mouseInfluence changes
