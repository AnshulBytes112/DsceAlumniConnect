...
export default function Alumni() {
    ...
    const allAlumni = realAlumni.length > 0 ? realAlumni.map(user => {
        ...
        return {
            id: parseInt(user.id),
            originalId: user.id,          // keep string id for routing
            originalProfile: user,         // pass full profile as router state
                                    ...
                                    <div className={viewMode === 'grid' ?
                                        ...
                                        {yearAlumni.map((alum) => (
                                            <motion.div
                                                key={alum.id}
                                                whileHover={{ y: -5 }}
                                                className={`group bg-white rounded-2xl border border-dsce-blue/5 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden ${viewMode === 'list' ? 'flex items-center p-4 gap-6' : 'p-6'
                                                    }`}
                                            >
                                                {viewMode === 'grid' ? (
                                                    <>
                                                        <div className="flex justify-between items-start mb-6">
                                                            ...
                                                            <div className="flex gap-2">
                                                                <button
                                                                    onClick={() => alum.linkedin && alum.linkedin !== '#' && window.open(alum.linkedin.startsWith('http') ? alum.linkedin : `https://${alum.linkedin}`, '_blank')}
                                                                ...
                                                                <button
                                                                    onClick={() => alum.email && window.open(`mailto:${alum.email}`)}
                                                        ...
                                                        <div className="pt-4 border-t border-gray-100 mt-4">
                                                            ...
                                                            <div className="grid grid-cols-2 gap-3">
                                                                <Button
                                                                    id={`profile-btn-${alum.id}`}
                                                                    variant="outline"
                                                                    className="w-full border-dsce-blue/20 text-dsce-blue hover:bg-dsce-blue/5"
                                                                    onClick={() => navigate(`/alumni/${(alum as any).originalId ?? alum.id}`, { state: { profile: (alum as any).originalProfile } })}
                                                                >
                                                                    Profile
                                                                </Button>
                                                                <Button
                                                    ...
                                                    <>
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            <Button
                                                                variant="ghost"
                                                                size="sm"
                                                                className="text-gray-500 hover:text-dsce-blue"
                                                                onClick={() => alum.email && window.open(`mailto:${alum.email}`)}
                                                                title="Send email"
                                                            >
