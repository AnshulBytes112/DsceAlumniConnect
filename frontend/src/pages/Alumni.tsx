...
export default function Alumni() {
    ...
    const filteredAlumni = allAlumni.filter(alum => {
        ...
        if (selectedSkills.length > 0 && 'isReal' in alum && alum.isReal) {
            ...
            // For mock alumni, they don't have skills data, so exclude them
            matchesSkills = false;
        }

        return matchesSearch && matchesYear && matchesDepartment && matchesCompany && matchesSkills;
    });
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-dsce-bg-light via-dsce-bg-cream to-dsce-bg-light">
            ...
            <MotionWrapper className="max-w-7xl mx-auto px-4 -mt-10 relative z-20 pb-20">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-2xl shadow-xl border border-dsce-blue/5 p-4 mb-12" >
                    <div className="flex flex-col md:flex-row gap-4 items-center">
                        ...
                        <div className="flex gap-3 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 no-scrollbar">
                            <select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-dsce-blue/20 text-gray-700 min-w-[140px] cursor-pointer hover:bg-gray-100 transition-colors" >
                                <option value="all">All Batches</option>
                                {years.map(year => <option key={year} value={year}>{year}</option>)}
                            </select>

                            <select value={selectedDepartment} onChange={(e) => setSelectedDepartment(e.target.value)} className="px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-dsce-blue/20 text-gray-700 min-w-[160px] cursor-pointer hover:bg-gray-100 transition-colors" >
                            ...
                            <button 
                                ...
                                className="flex items-center gap-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl hover:bg-gray-100 transition-colors min-w-[140px] justify-center"
                            >
                            ...
                            <div className="flex bg-gray-50 rounded-xl p-1 border border-gray-200">
                                <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-white shadow-sm text-dsce-blue' : 'text-gray-400 hover:text-gray-600'}`} title="Grid View">
                                    <Users className="h-5 w-5" />
                                </button>
                                <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-white shadow-sm text-dsce-blue' : 'text-gray-400 hover:text-gray-600'}`} title="List View">
                                    <Filter className="h-5 w-5" />
                                </button>
                                <button onClick={() => setViewMode('map')} className={`p-2 rounded-lg transition-all ${viewMode === 'map' ? 'bg-white shadow-sm text-dsce-blue' : 'text-gray-400 hover:text-gray-600'}`} title="Map View">
                    ...
                    <AnimatePresence>
                        {hasActiveFilters && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="flex items-center gap-2 mt-4 pt-4 border-t border-gray-100 overflow-hidden" >
                                <span className="text-sm text-gray-500 mr-2">Active Filters:</span>
                                {searchTerm && <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-dsce-blue/10 text-dsce-blue">Search: {searchTerm}<button onClick={() => setSearchTerm('')} className="ml-2 hover:text-dsce-blue/70"><X className="h-3 w-3" /></button></span>}
                                {selectedYear !== 'all' && <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-dsce-blue/10 text-dsce-blue">Batch: {selectedYear}<button onClick={() => setSelectedYear('all')} className="ml-2 hover:text-dsce-blue/70"><X className="h-3 w-3" /></button></span>}
                                {selectedDepartment !== 'all' && <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-dsce-blue/10 text-dsce-blue">Dept: {selectedDepartment}<button onClick={() => setSelectedDepartment('all')} className="ml-2 hover:text-dsce-blue/70"><X className="h-3 w-3" /></button></span>}
                                {selectedCompany !== 'all' && <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-dsce-blue/10 text-dsce-blue">Company: {selectedCompany}<button onClick={() => setSelectedCompany('all')} className="ml-2 hover:text-dsce-blue/70"><X className="h-3 w-3" /></button></span>}
                                {selectedSkills.length > 0 && <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-dsce-blue/10 text-dsce-blue">Skills: {selectedSkills.join(', ')}<button onClick={() => setSelectedSkills([])} className="ml-2 hover:text-dsce-blue/70"><X className="h-3 w-3" /></button></span>}
                                <button onClick={clearFilters} className="text-xs text-red-500 hover:text-red-600 font-medium ml-auto">Clear All</button>
                            </motion.div>
                        )}
                    </AnimatePresence>
                </motion.div>
                ...
                {loading ? (
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
                        <AlumniMap alumni={filteredAlumni} />
                    </motion.div>
                                        ...
                                        {yearAlumni.map((alum) => (
                                            <motion.div key={alum.id} whileHover={{ y: -5 }} className={`group bg-white rounded-2xl border border-dsce-blue/5 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden ${viewMode === 'list' ? 'flex items-center p-4 gap-6' : 'p-6'}`}>
                                                {viewMode === 'grid' ? (
                                                    <>
                                                        <div className="flex justify-between items-start mb-6">
                                                            <div className="relative">
                                                                <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-dsce-blue to-dsce-light-blue flex items-center justify-center text-white shadow-lg group-hover:scale-105 transition-transform duration-300">
                                                                    {alum.image ? <img src={alum.image} alt={alum.name} className="h-full w-full object-cover rounded-2xl" /> : <span className="text-2xl font-bold">{alum.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}</span>}
                                                                </div>
                                                        ...
                                                        <div className="mb-4">
                                                            ...
                                                            <div className="space-y-2">
                                                                <div className="flex items-center text-gray-600 text-sm"><Briefcase className="h-4 w-4 mr-2 text-gray-400" />{alum.position}</div>
                                                                <div className="flex items-center text-gray-600 text-sm"><Building2 className="h-4 w-4 mr-2 text-gray-400" />{alum.company}</div>
                                                                <div className="flex items-center text-gray-600 text-sm"><MapPin className="h-4 w-4 mr-2 text-gray-400" />{alum.location}</div>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <>
                                                        <div className="h-16 w-16 flex-shrink-0 rounded-2xl bg-gradient-to-br from-dsce-blue to-dsce-light-blue flex items-center justify-center text-white shadow-md">{alum.image ? <img src={alum.image} alt={alum.name} className="h-full w-full object-cover rounded-2xl" /> : <span className="text-2xl font-bold">{alum.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}</span>}</div>
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex items-center gap-3 mb-1"><h3 className="text-lg font-bold text-dsce-text-dark truncate">{alum.name}</h3><span className="px-2 py-0.5 rounded-full bg-dsce-blue/10 text-dsce-blue text-xs font-medium">{alum.department}</span></div>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200" >
                        ...
                        <p className="text-gray-500 mb-8 max-w-md mx-auto">We couldn't find any alumni matching your search criteria. Try adjusting your filters or search terms.</p>
                        <Button onClick={clearFilters} variant="outline" className="border-dsce-blue text-dsce-blue hover:bg-dsce-blue/5">Clear All Filters</Button>
                    </motion.div>
                )}
            </MotionWrapper>
        </div>
